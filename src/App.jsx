...
export default function App() {
  ...
  useEffect(() => {
    function handleResize() {
      ...
      if (window.innerWidth <= 800 && (currentChatFriend || currentGroup)) setShowSidebar(false);
    }
    window.addEventListener("resize", handleResize);
    return () => window.removeEventListener("resize", handleResize);
  }, [currentChatFriend, currentGroup]);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        subscribeGroupRequests(currentUser.uid);
      } else {
        setFriendRequests([]);
        setFriends([]);
        setBlockedUsers([]);
  ...
  function subscribeGroups(uid) {
    ...
    return onSnapshot(query(collection(db, "groups"), where("members", "array-contains", uid)), (snap) => {
      setGroups(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
  }
  function subscribeGroupRequests(uid) {
    // Listen for group invitations/requests
    return onSnapshot(query(collection(db, "groupRequests"), where("to", "==", uid)), (snap) => {
      setGroupRequests(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
  }
  async function createGroup(groupName, memberUsernames) {
    if (!user) return;
    const members = [user.uid];
    for (let uname of memberUsernames) {
      const q = query(collection(db, "users"), where("username", "==", uname.trim()));
    ...
    setCurrentGroup({ id: groupDoc.id, members, name: groupName });
    setCurrentChatFriend(null);
    if (isMobile) setShowSidebar(false);
  }
  async function sendGroupRequest(groupId, toUid) {
    await addDoc(collection(db, "groupRequests"), {
      groupId, to: toUid, from: user.uid, status: "pending", createdAt: serverTimestamp()
    });
  }
  async function acceptGroupRequest(reqId, groupId) {
    // Add to group members
    await updateDoc(doc(db, "groups", groupId), { members: arrayUnion(user.uid) });
    await deleteDoc(doc(db, "groupRequests", reqId));
  }
  async function rejectGroupRequest(reqId) {
    await deleteDoc(doc(db, "groupRequests", reqId));
  }

  // --- MAIN MESSAGE LOADING LOGIC
  useEffect(() => {
    if (!user) {
      setMessages([]);
      return;
    ...
    if (currentGroup) {
      // GROUP CHAT
      const messagesRef = collection(db, "groupChats", currentGroup.id, "messages");
    ...
    setMessages([]);
  // eslint-disable-next-line
  }, [user, currentChatFriend, currentGroup, chatPasswords]);

  async function sendMessage() {
    if ((!newMessage.trim() && !newMedia) || (!currentChatFriend && !currentGroup) || !user) return;
    if (newMedia) {
  ...
  async function deleteMessage(messageId, message, onlySelf = false) {
    if ((!currentChatFriend && !currentGroup) || !user) return;
    try {
      if (currentGroup) {
        const msgRef = doc(db, "groupChats", currentGroup.id, "messages", messageId);
        if (onlySelf) {
  ...
  async function cleanChat() {
    if (!user) return;
    if (currentGroup) {
      const msgsRef = collection(db, "groupChats", currentGroup.id, "messages");
  ...
  async function handleTyping(e) {
    ...
    if (currentGroup) {
      const typingRef = doc(db, "groupChats", currentGroup.id);
      try {
        await updateDoc(typingRef, {
          [`typing.${user.uid}`]: true,
        });
  ...
  useEffect(() => {
    ...
    if (currentGroup) {
      const typingDocRef = doc(db, "groupChats", currentGroup.id);
    ...
    setTypingStatus({});
  }, [user, currentChatFriend, currentGroup]);
  ...
  // ----------- MOBILE: Meeting/Settings Dropdowns -----------
  function MobileDropdowns() {
    // Meeting Dropdown: Add friend, create group, friend/group lists, requests
    ...
    return (
      <>
        {/* Meeting Panel Dropdown */}
        <div style={{ margin: "0 14px 18px 14px" }}>
          <button
            style={{
              width: "100%",
              padding: "12px 0",
            ...
            onClick={() => setShowMeetingDropdown(s => !s)}
          >
            Meeting Panel ▼
          </button>
          {showMeetingDropdown && (
            <div style={{
              ...
              {/* Add Friend */}
              <button
                style={{ width: "100%", marginBottom: 8, background: "#219653", color: "#fff", border: "none", borderRadius: 6, padding: 8, fontWeight: 600 }}
                onClick={() => setAddFriendOpen(s => !s)}
              >Add Friend</button>
              {addFriendOpen && <AddFriend sendFriendRequest={sendFriendRequest} />}
              {/* Create Group */}
              <button
                style={{ width: "100%", marginBottom: 8, background: "#0d6efd", color: "#fff", border: "none", borderRadius: 6, padding: 8, fontWeight: 600 }}
                onClick={() => setCreateGroupOpen(s => !s)}
              >Create Group</button>
              {createGroupOpen && (
                <form onSubmit={e => {
                  e.preventDefault();
                  createGroup(groupName, groupMembers.split(","));
                  setGroupName(""); setGroupMembers(""); setCreateGroupOpen(false);
                }}>
                  <input
                    type="text"
                    ...
                    style={{ width: "100%", marginBottom: 5, padding: 6 }}
                  />
                  <input
                    type="text"
                    placeholder="Members (comma usernames)"
                    value={groupMembers}
                    onChange={e => setGroupMembers(e.target.value)}
                    style={{ width: "100%", marginBottom: 5, padding: 6 }}
                  />
                  <button type="submit" style={{ width: "100%", background: "#219653", color: "#fff", border: "none", borderRadius: 4, padding: 7 }}>
                    Create
                  </button>
                </form>
              )}
              ...
              {groupRequests.length > 0 && (
                <>
                  ...
                  {groupRequests.map(req => (
                    <div key={req.id} style={{ background: "#fff", borderRadius: 7, padding: 8, marginBottom: 6 }}>
                      <b>Group: {groups.find(g => g.id === req.groupId)?.name || req.groupId}</b>
                      <button onClick={() => acceptGroupRequest(req.id, req.groupId)} style={{ marginLeft: 9, background: "#219653", color: "#fff", border: "none", borderRadius: 4, padding: "2px 8px", fontSize: 12 }}>Accept</button>
                      <button onClick={() => rejectGroupRequest(req.id)} style={{ marginLeft: 5, background: "#dc3545", color: "#fff", border: "none", borderRadius: 4, padding: "2px 8px", fontSize: 12 }}>Reject</button>
                    </div>
                  ))}
                </>
              )}
              {/* Friends List */}
              <h4 style={{ marginTop: 17 }}>Friends</h4>
              ...
              {/* Groups List */}
              <h4 style={{ marginTop: 17 }}>Groups</h4>
              {groups.length === 0 ? (
                ...
                groups.map(group => (
                  <div
                    ...
                    style={{
                      padding: 8,
                      background: currentGroup?.id === group.id ? "#e3f2fd" : "#fff",
                      borderRadius: 7,
                      border: currentGroup?.id === group.id ? "2px solid #219653" : "1px solid #eee",
                      marginBottom: 5,
                      cursor: "pointer",
                      fontWeight: 600
                    }}
        ...
        <div style={{ margin: "0 14px" }}>
          <button
            ...
            onClick={() => setShowSettingsDropdown(s => !s)}
          >
            Settings ▼
          </button>
          {showSettingsDropdown && (
            <div style={{
              ...
              <button
                ...
                style={{
                  width: "100%", padding: 9, borderRadius: 6, background: "#eee", color: "#219653", fontWeight: 600, border: "none", marginBottom: 7
                }}
              >Switch Theme</button>
              <button
                onClick={() => { setShowProfileUpload(true); setShowSettingsDropdown(false); }}
                style={{
                  width: "100%", padding: 9, borderRadius: 6, background: "#219653", color: "#fff", fontWeight: 600, border: "none", marginBottom: 7
                }}
              >📷 Profile Photo</button>
              <button
                onClick={() => { setShowPasswordChange(true); setShowSettingsDropdown(false); }}
                style={{
                  width: "100%", padding: 9, borderRadius: 6, background: "#6c757d", color: "#fff", fontWeight: 600, border: "none", marginBottom: 7
                }}
              >🔐 Change Password</button>
              <button
                onClick={() => { setShowBlockedUsers(true); setShowSettingsDropdown(false); }}
                style={{
                  width: "100%", padding: 9, borderRadius: 6, background: "#dc3545", color: "#fff", fontWeight: 600, border: "none", marginBottom: 7
                }}
              >🚫 Blocked Users ({blockedUsers.length})</button>
              <button
              ...
              >Logout</button>
              <button
                onClick={() => { setShowAboutUs(true); setShowSettingsDropdown(false); }}
                style={{
                  width: "100%", padding: 9, borderRadius: 6, background: "#0d6efd", color: "#fff", fontWeight: 600, border: "none", marginBottom: 7
                }}
              >About Us</button>
              <button
                onClick={() => { setShowDeleteAccount(true); setShowSettingsDropdown(false); }}
                style={{
                  width: "100%", padding: 9, borderRadius: 6, background: "#ff3333", color: "#fff", fontWeight: 600, border: "none", marginBottom: 7
                }}
  ...
  return (
    <ThemeContext.Provider value={{ theme, handleThemeSwitch }}>
      <div className="main-app" style={{
        ...
        {/* Header: mobile - show only in lobby, desktop always */}
        {!(isMobile && (currentChatFriend || currentGroup)) && <UniversalHeader />}

        {/* MOBILE MEETING PANEL & SETTINGS dropdowns */}
        {isMobile && !currentChatFriend && !currentGroup && (
          <div style={{ width: "100vw", paddingTop: "2rem" }}>
            <MobileDropdowns />
          </div>
        )}

        {/* SIDEBAR (desktop-only, for navigation) */}
        {!isMobile && (
          <div style={{
            width: 320,
        ...
        <div style={{
          ...
          {(currentChatFriend || currentGroup) ? (
            <>
              {/* Mobile Chat Header: back, friend/group name, 3-dot menu */}
              {isMobile && (
                <div style={{
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "space-between",
                  background: theme === "dark" ? "#181c21" : "#fff",
                  ...
                  <button
                    onClick={() => { setCurrentChatFriend(null); setCurrentGroup(null); setShowSidebar(true); }}
                    style={{
                      fontSize: 23,
                  ...
                  >←</button>
                  {currentChatFriend && <ChatFriendHeader friendId={currentChatFriend.uid} onlineUsers={onlineUsers} theme={theme} />}
                  {currentGroup && (
                    <div style={{ display: "flex", alignItems: "center" }}>
                      <div style={{ marginRight: 8 }}>
                        <span style={{
                          ...
                          color: "#fff",
                          borderRadius: "50%",
                          width: 37, height: 37, display: "inline-flex", alignItems: "center", justifyContent: "center",
                          fontWeight: 900, fontSize: 18
                        }}>
                          {currentGroup.name ? currentGroup.name[0] : "G"}
                        </span>
                      </div>
                      <div>
                        <h3 style={{ margin: 0, color: '#333', fontSize: 17 }}>{currentGroup.name}</h3>
              ...
              <div style={{
                ...
                {requirePassword ? (
                  ...
                  <>
                    ...
                    {/* Typing indicator for 1:1 or group */}
                    ...
                    {currentGroup && Object.entries(typingStatus).filter(([uid, val]) => val && uid !== user.uid).map(([uid, val], i) =>
                      <div key={uid} style={{
                        padding: '10px',
                        backgroundColor: 'rgba(33,150,83,0.12)',
                        borderRadius: '15px',
                        ...
                        <em style={{ color: '#219653' }}>{uid} typing...</em>
                      </div>
                    )}
                  </>
                )}
          ...
          {/* Settings Modal, Profile Upload, Change Password, Blocked Users, Set Chat Password Modal, About Us, Delete Account */}
          {showSettings && (
            <div style={{
              position: "fixed",
          ...
          {showDeleteAccount && (
            <div style={{
              position: "fixed",
              left: 0, top: 0, width: "100vw", height: "100vh", zIndex: 5004,
            ...
            >
              <div style={{
                ...
                <p style={{ margin: "10px 0", color: "#888" }}>
                  This action is <b>irreversible</b>. Enter your password to confirm.
                </p>
                <input
                  type="password"
                  ...
                  placeholder="Current password"
                  style={{ width: "100%", padding: 8, borderRadius: 5, border: "1px solid #ccc", marginBottom: 9 }}
                />
                <button
                  onClick={handleDeleteAccount}
                  style={{
                    width: "100%", background: "#ff3333", color: "#fff", border: "none", borderRadius: 5, padding: 10, fontWeight: 700
                  }}
                >Delete Account</button>
                <button
                  onClick={() => setShowDeleteAccount(false)}
                  style={{
                    width: "100%", background: "#ccc", color: "#23272f", border: "none", borderRadius: 5, padding: 10, fontWeight: 700, marginTop: 6
                  }}
                >Cancel</button>
                {deleteAccountError && <div style={{ color: "#e74c3c", marginTop: 7 }}>{deleteAccountError}</div>}
              </div>
            </div>
          )}
        </div>
