// App.jsx
// Darknet Chat App - Updated with new features and comments

import React, { useState, useEffect, useRef } from "react";
import {
  User, Send, Plus, Settings as SettingsIcon, LogOut, Moon, Sun, Image, Smile, Edit3, Trash2, Check, CheckCheck,
  MessageSquare, Users, Search, MoreVertical, X, UserPlus, UserMinus, Trash, Info, Wallpaper
} from "lucide-react";
import { initializeApp } from "firebase/app";
import {
  getFirestore, getDocs, collection, addDoc, query, where, updateDoc, doc, onSnapshot, arrayUnion, deleteDoc, orderBy, serverTimestamp
...
@media (max-width: 768px) {
  ...
  .chat-list {display: ${window.innerWidth < 768 ? 'none' : 'block'};}
}
...
const Login = ({ onLogin, onShowRegister }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      // Email is constructed from username, always in lowercase
...
// Sidebar: now displays profile photo at the top
const Sidebar = ({
  ...
  <div className="sidebar w-16 bg-gray-800 flex flex-col items-center py-4 h-screen gap-4" style={{gap: 24}}>
    <button onClick={onProfile} className="w-12 h-12 mb-2 sidebar-profile flex items-center justify-center">
      {user.photoURL
        ? <img src={user.photoURL} className="w-12 h-12 rounded-full object-cover" alt="Profile" />
        : <User className="w-12 h-12 rounded-full bg-blue-400 text-white" />}
    </button>
    <div className="flex flex-col sidebar-icons gap-6" style={{gap: 24}}>
      <button onClick={onFriends} title="Chats"><Users className="text-white" /></button>
...
// Chat List (no changes needed for username case, as usernames are already saved lowercase)
const ChatList = ({ currentUser, onSelectChat, activeChat }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      async (snap) => {
        ...
        fetched.sort((a, b) => b.lastMessageTimestamp - a.lastMessageTimestamp);
        setChats(fetched);
      }
    );
    return () => unsub();
  ...
  return (
    <div className="chat-list w-80 bg-white dark:bg-gray-800 border-r dark:border-gray-700 flex flex-col">
...
// --- REALTIME Chat Window with Wallpaper/Theme/Unfriend ---
    ...
    <div className={`flex-1 flex flex-col bg-white dark:bg-gray-900`} style={wallpaper ? {backgroundImage: `url(${wallpaper})`, backgroundSize: 'cover'} : {}}>
      <div className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 p-4 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 relative">
          ...
          {showMenu && (
            <div className="absolute right-0 top-10 bg-white dark:bg-gray-800 shadow rounded z-50 py-1 min-w-[200px]">
...
// User Profile: display photo, username, and bio
...
const Settings = ({ onAbout, onLogout }) => {
  ...
  const handleChangeUsername = () => {
    setInfo("Username changed successfully (mock)");
  };
  const handleChangePassword = () => {
    setInfo("Password changed successfully (mock)");
  };
  const handleBlock = () => {
    setInfo(`Blocked user ${blockUser} (mock)`);
  };
  const handleUnblock = () => {
    setInfo(`Unblocked user ${unblockUser} (mock)`);
  };
  ...
  return (
    <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
      ...
      <div className="mb-4">
        ...
        />
        <button onClick={handleChangeUsername} className="bg-blue-500 text-white px-3 py-1 rounded">Change</button>
      </div>
      <div className="mb-4">
        ...
        />
        <button onClick={handleChangePassword} className="bg-blue-500 text-white px-3 py-1 rounded">Change</button>
      </div>
      <div className="mb-4">
        <label className="block mb-1">Block user by username</label>
        <input
          value={blockUser}
      ...
      <div className="mb-4">
        <label className="block mb-1">Unblock user by username</label>
        <input
          value={unblockUser}
          onChange={e => setUnblockUser(e.target.value)}
          className="w-full px-3 py-2 border rounded mb-2"
          placeholder="Username"
        />
        <button onClick={handleUnblock} className="bg-green-500 text-white px-3 py-1 rounded">Unblock</button>
      </div>
