...
const Message = ({
  ...
  // If "deletedFor" includes currentUser.uid, do not render (handled in parent map)
  return (
    <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-4 group`}>
      ...
      >
        ...
        <div className="flex items-center justify-between mt-1 text-xs opacity-70">
          ...
          {menuOpen && (
            <div
              className="absolute top-8 right-0 bg-white dark:bg-gray-900 border rounded shadow-xl z-50 min-w-[140px]"
            ...
            >
              ...
              {!isOwn && (
                <button
                  className="w-full flex items-center px-4 py-2 text-red-500 hover:bg-gray-100 dark:hover:bg-gray-800"
                  onClick={() => { setMenuOpen(false); onBlock(message.senderId); }}
                >
                  Block User
                </button>
              )}
              <button
                className="w-full flex items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-800"
                onClick={() => { setMenuOpen(false); onSchedule(message); }}
              >
                Schedule Message
              </button>
            </div>
...
const ChatWindowMobileHeader = ({ onBack, activeChat }) => (
  <div className="flex items-center p-4 bg-white dark:bg-gray-800 border-b dark:border-gray-700">
    <button onClick={onBack} className="mr-2 p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
      <X size={22} />
    </button>
